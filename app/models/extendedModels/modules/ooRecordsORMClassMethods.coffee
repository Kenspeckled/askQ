...
_ = require 'lodash'
publishSubscribe = require 'models/modules/pubSub.coffee'
...
generateUniqueId = ->
  ...
  newIdPromise = _utilities.promiseWhile condition, ->
    new Promise (resolve) ->
      id = generateId()
      self.client.lrange self.name + '#uniqueQueuedIds', 0, -1, (error, idsArray) ->
        if _.includes(idsArray, id)
          id = generateId()
          resolve()
        else
          self.client.rpush self.name + '#uniqueQueuedIds', id, (error, response) ->
            uniqueId = true
            resolve(id)
...
indexSortedSet = (setKey, attr) ->
  ...
  sortPromise = new Promise (resolve, reject) =>
    @client.sort setKey, 'by', @name + ':*->' + attr, 'alpha', 'store', listKey, (error, newLength) ->
      resolve(newLength)
    ...
    new Promise (resolve, reject) =>
      ...
      multi.exec (error, ids) =>
        addToSet = _.map ids, (id, i) =>
          new Promise (r) =>
            @client.zadd setTmpKey, listLength - i, id, (res) ->
              r()
        resolve Promise.all(addToSet)
  addToTmpSetPromise.then =>
    new Promise (resolve) =>
      @client.rename setTmpKey, setKey, (res) =>
        resolve()
  ...
  for word in words.split /\s/
    ...
    for char in word
      ...
      indexPromiseFn = (wordSegmentKey, id) =>
        new Promise (resolve) =>
          @client.zadd wordSegmentKey, 1, id, (res) ->
            resolve()
...
removeIndexedSearchableString = (attr, words, id) ->
  ...
  for word in words.split /\s/
    ...
    for char in word
      ...
      indexPromiseFn = (wordSegmentKey, id) =>
        new Promise (resolve) =>
          @client.zrem wordSegmentKey, id, (res) ->
            resolve()
      indexPromises.push indexPromiseFn(wordSegmentKey, id)
  Promise.all(indexPromises)
...
writeAttributes = (props) ->
  ...
  writePromise = idPromise.then (id) ->
    ...
    new Promise (resolve) ->
      self.client.hmset objectIdentifier, props, (err, res) ->
        resolve()
  indexPromise = writePromise.then ->
    ...
    indexPromiseFn = (sortedSetName, attributeName) ->
      ...
      new Promise (resolve) ->
        self.client.zadd sortedSetName, largestSortedSetSize, props.id, (error, res) ->
...
clearUniqueQueuedIds = ->
  @client.del @name + '#uniqueQueuedIds'
...
processWriteQueue = ->
  ...
  processPromise = _utilities.promiseWhile condition, ->
    writePromise = new Promise (resolve, reject) ->
      self.client.rpop self.name + "#TmpQueue", (error, tmpId) ->
        if tmpId
          self.client.hgetall self.name + "#TmpQueueObj:" + tmpId, (err, props) ->
            self.client.del self.name + "#TmpQueueObj:" + tmpId
            if props
...
addToWriteQueue = (props) ->
  ...
  p = new Promise (resolve) ->
    self.client.hmset self.name + "#TmpQueueObj:" + tmpId, props, (err, res) =>
      self.client.lpush self.name + "#TmpQueue", tmpId, (error, newListLength) =>
        resolve(tmpId)
  p.then (tmpId) ->
...
createIntegerSubset = (integerSortedSetName, tempIntegerKey, minValue, maxValue) ->
  self = this
  new Promise (resolve) ->
    self.client.zrangebyscore integerSortedSetName, minValue, maxValue, (error, resultList) ->
      multi = self.client.multi()
...
findKeywordsInAnyFields = (fields, keywords, weightOptions) ->
    ...
    unionKeyPromises = _.map keywords, (keyword) =>
      ...
      unionKeyPromise = new Promise (resolve) =>
        @client.zunionstore unionKey, keyNames.length, _.map(keyNames, 'name')..., 'weights', _.map(keyNames, 'weight')..., ->
          resolve unionKey
...
OORecordsORM =
  ...
  find: (id) ->
    ...
    foundPromise = new Promise (resolve, reject) =>
      @client.hgetall @name + ":" + id, (error, props) =>
        if props
  ...
  where: (args) ->
    ...
    for option in Object.keys(args)
      ...
      switch @attributes[option].dataType
        ...
        when 'boolean'
          sortedSetKeys.push  name: @name + "#" + option + ":" + optionValue
